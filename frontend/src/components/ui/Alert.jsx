/**
 * Alert / banner de notificación.
 * @param {'success'|'error'|'warning'|'info'} type
 * @param {string} title — opcional
 * @param {string|ReactNode} children
 * @param {function} onClose — si se pasa, muestra botón de cerrar
 */
...
export default function Alert({ type = 'info', title, children, onClose }) {
  const s = styles[type]
  return (
    <div className={`flex gap-3 rounded-lg border p-4 ${s.wrap}`} role="alert">
      ...
      {onClose && (
        <button onClick={onClose} className="shrink-0 self-start opacity-60 hover:opacity-100">
