...
\begin{longtable}{p{0.32\textwidth} p{0.18\textwidth} p{0.40\textwidth}}
    ...
    \midrule
    Fixtures precargados & 5 & Departamentos, licenciaturas, posgrados, áreas (\texttt{catalogos/fixtures/}) \\
    \bottomrule
\end{longtable}
...
La ejecución completa de la \emph{suite} arroja \textbf{196 pruebas exitosas de 196} (100\,\%) y una \textbf{cobertura de código del 94\,\%} medida con \texttt{coverage.py} (comando \texttt{pytest --cov=. --cov-report=html}). El reporte HTML detallado se incluye en la carpeta de entregables bajo \texttt{cobertura/}.
...
\begin{figure}[!ht]
    ...
    \caption{Constructor de formularios de autoevaluación (\emph{form-builder}) con secciones y preguntas ponderadas.}
