...
\paragraph{Entorno virtual}\mbox{}\\
\begin{quote}
\ttfamily
cd backend \\
python -m venv .venv \\
...
\paragraph{Archivo \texttt{.env}}\mbox{}\\
...
\paragraph{Datos iniciales}\mbox{}\\
...
\paragraph{Servidor de desarrollo del backend}\mbox{}\\
\begin{quote}
\ttfamily
...
\begin{itemize}
    ...
    \item \textbf{\texttt{seed\_demo.py} falla con ``La suma de pesos de las secciones debe ser 100\,\%''}: en algunas versiones del script las secciones se crean sin \texttt{peso}, lo que hace fallar la publicación del formulario. Como \emph{workaround} temporal, edite \texttt{backend/scripts/seed\_demo.py} y añada \texttt{peso=Decimal("50")} a la creación de cada una de las dos \texttt{Seccion.objects.create(\ldots)}, luego ejecute \texttt{python manage.py flush --noinput} y vuelva a lanzar el seed.
